scotchApp.controller('opticsMesiraHanutController', function ($scope, $routeParams, $route, $rootScope, mainService) {
...
<MOKED>'+ MOKED + '</MOKED>\
    ...
    angular.element(document).ready(function () {
        ...
        getHanuiot(hanuxML);
        if ($routeParams.From == "1" || $routeParams.From == "2" || $routeParams.From == "3") {
   ...
   <soapenv:Body>\
    ...
    $scope.onCheckBarCode = function () {
        ...
        if (currentOption == null || currentOption == "-1") {
            navigator.notification.alert('לא בחרת חנות מהרשימה');
        }
        else {
            if (barcode.length == 0) {
                    ...
                    mainService.send32(kodmesiraValue, barcode).then(function (response) {
                        ...
                        if (data.RESULT == '0') {
                            ...
                            if (data.RQW == "1") {
                                ...
                                var tempHanut = $('.area').find(":selected").text();
                                var arr = tempHanut.split(',');
                                KodHanut = arr[1].trim();

                                //If item is EMS
                                if (barcode[0] == "E" && barcode[1] == "E") {
                                    GoToEms(ORG, barcode, mesira, countPictures, data.MEM, KodHanut);
                                }
                                else {
                                    GoToRegularWeight(ORG, barcode, mesira, countPictures, isPalet, data.MEM, KodHanut);
                                }
                            }
                            else {
                                kodMesira = "1";
    ...
    function GoToEms(ORG, barcode, kodmesira, countPictures, MEM, KodHanut) {
        ...
        location.href = "#/weightItem/originalWeight/" + ORG + "/barcode/" + barcode + "/kodmesira/" + kodmesira + "/countPictures/" + countPictures + "/MEM/" + MEM + "/From/3/Count/" + countDelivered + "/KodHanut/" + KodHanut;
    }
    function GoToRegularWeight(ORG, barcode, kodmesira, countPictures, isPalet, MEM, KodHanut) {
        ...
        location.href = "#/weightNormal/originalWeight/" + ORG + "/barcode/" + barcode + "/kodmesira/" + kodmesira + "/countPictures/" + countPictures + "/isPalet/" + isPalet + "/MEM/" + MEM + "/From/3/Count/" + countDelivered + "/KodHanut/" + KodHanut;
    }
